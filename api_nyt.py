...
API_KEY = "NYT_API_KEY" #Here your API KEY_ You can find it on their NYT developer site
AÑO = 2025
MESES = range(1, 13) # Cobertura total del año

#EXTRACCION DE DATOS
def descargar_archivo_mensual(api_key, year, month):
    url = f"https://api.nytimes.com/svc/archive/v1/{year}/{month}.json?api-key={api_key}"
    print(f"📡 Recuperando inventario total de: {year}-{month:02d}...")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"⚠️ Error {response.status_code} en mes {month}. Verifique API Key.")
            return []
...
def filtrar_por_peru(documentos):
    ...
    for doc in documentos:
        ...
        # Manejar el caso donde 'keywords' podría ser None
        ...
        # Filtro de seguridad: captura variaciones semánticas
...
for mes in MESES:
    # 1. Traer todo el mes
    total_docs_mes = descargar_archivo_mensual(API_KEY, AÑO, mes)
    articulos_totales_procesados += len(total_docs_mes)

    # 2. Filtrar lo relevante para Perú
...
if corpus_peru_2025:
    ...
    # Ordenar cronológicamente (Enero -> Diciembre)
    ...
    print("\n" + "="*40)
    print(f"📊 RESUMEN DE LA INVESTIGACIÓN:")
    print(f"   - Artículos del NYT analizados: {articulos_totales_procesados}")
    print(f"   - Noticias filtradas sobre Perú: {len(df_final)}")
    print("="*40)

    # DESCARGA AUTOMÁTICA
    print(f"📥 Descargando '{nombre_file}' a tu equipo...")
    files.download(nombre_file)
else:
    print("❌ No se encontraron registros de Perú en el archivo 2025 del NYT.")
